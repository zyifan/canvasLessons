...
COREHTML5.topLeftTimeBasedMotion = function () {
    var topCanvas = document.querySelector('#topCanvas'), //获取canvas对象
        ...
        animateButton = document.querySelector('#topAnimateButton'), // 播放按钮
    ...
    function updateTimeBased(time) {
        ...
        while (i--) {
            ...
            // 根据时间计算每帧需要运动的距离 deltaX，( 1s运行 velocityX 长的距离 / 绘制时间 = 1帧时间内可以运行多远deltaX )
    ...
    function calculateFps(now) {
        elapsedTime = now - lastTime;// 一帧所用的时间间隔

        fps = 1000 / elapsedTime;// 1s运行了多少帧 ( 1s/一帧所用的时间 )
        lastTime = now;
    }
    ...
    drawBackground();
    startTime = +new Date;
    setInterval(animateTopLeft, 1000 / 60); // 1s播放60帧
}();